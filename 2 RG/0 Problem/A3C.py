...
class ACNet(object):
    ...
    def _build_net(self, scope):  # 网络定义
        ...
        with tf.variable_scope('critic'):
            ...
            l_c = tf.layers.dense(l_c1, self.para.units_c, tf.nn.relu6, kernel_initializer=w_init, name='lc')
        ...
        while self.para.GLOBAL_EP < self.para.MAX_GLOBAL_EP:
            ...
            for ep_t in range(self.para.MAX_EP_STEP):  # MAX_EP_STEP每个片段的最大个数
                ...
                if done:  # 每个片段结束，输出一下结果
                    if self.name == 'W_0':
                        print(np.array(ep_a[::50]))
                    self.para.GLOBAL_RUNNING_R.append(ep_r)
                    if self.para.GLOBAL_EP % 1 == 0:
                        range_error = self.env_l.state[1] * self.env_l.R0 - self.env_l.range_need
                        print(
                            self.name,
                            "Ep:", self.para.GLOBAL_EP,
                            "range_error: %.4f" % range_error,
                            "hot: %.4f" % self.env_l.state[4],
