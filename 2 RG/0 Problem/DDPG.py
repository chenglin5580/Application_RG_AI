...
class DDPG(object):
    def __init__(
            ...
            var_liner_times=1000,  # 探索值经历多少次学习变成e_end
            var_init=1,  # 表示1倍的幅值作为初始值
        ...
        self.var_init = var_init  # 初始的探索值
        self.var = self.var_init
        self.var_end = var_end
        self.e_liner_times = var_liner_times
        self.train = train
    ...
    def choose_action(self, s):
        ...
        if self.train:
            action = np.clip(np.random.normal(action, abs(self.a_bound[0] - self.a_bound[1]) / 2 * self.var),
    ...
    def learn(self):
        if self.pointer < self.MEMORY_CAPACITY:
            ...
            self.var = max(self.var - (self.var_init - self.var_end) / self.var_liner_times,
                           self.var_end)
        ...
        for episode in range(episodes):
            ...
            while True:
                ...
                if done:
                    break
                step += 1
            print('Episode:', episode + 1, ' ep_reward: %.4f' % ep_reward, 'epsilon: %.3f' % RL.var)
        RL.model_save()
    else:
        pass
